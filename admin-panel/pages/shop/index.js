...
export default function ShopHome() {
  ...
  const fetchPlugs = async () => {
    try {
      console.log('🧪 Utilisation de l\'API de test locale...')
      
      let data
      try {
        // Utiliser l'API de test locale en premier
        const testResponse = await fetch(`/api/test-plugs?limit=50&t=${new Date().getTime()}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'Cache-Control': 'no-cache'
        ...
        if (testResponse.ok) {
          ...
          console.log('✅ API de test locale réussie:', data)
        } else {
          throw new Error(`Test API failed: HTTP ${testResponse.status}`)
        }
      } catch (testError) {
        console.log('❌ API de test locale échouée:', testError.message)
